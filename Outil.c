...
int bChaineEgale(char *sTexte1, char *sTexte2){
	int bEgale = nChaineLg(sTexte1) == nChaineLg(sTexte2);
	int uC;
	for(uC=1;bEgale && uC<=nChaineLg(sTexte1);uC++)
		bEgale=sTexte1[uC] == sTexte2[uC]; 
}//bChaineEgale
...
int bVoyellE2(char *sTexte,int *pnCodeCaractereRg){
	...
	int bVoyellE2=0;
	assert(sTexte!=0);
	Assert1("bVoyellE2",bCroit(0,*pnCodeCaractereRg,nChaineLg(sTexte)));
	cCode=sTexte[*pnCodeCaractereRg];
